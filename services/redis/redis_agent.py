...
Run: python redis_agent.py  (from services/) or uv run python services/redis_agent.py
"""
...
def get_llm():
    """Same LLM config as orchestrator (W&B Inference)."""
    return ChatOpenAI(
...
async def main():
    ...
    print("Redis agent (LangChain + redis-mcp-server → localhost:6379). Type 'quit' to exit.\n")
    while True:
        try:
